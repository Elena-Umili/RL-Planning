...
class DDQNAgent:
    ...
    def __init__(self, env, buffer, epsilon=0.05, batch_size=32):
        ...
        self.env = env
        self.network = QNetwork(env=env, n_hidden_nodes=32, encoder=self.encoder)
    ...
    def planner_action(self, depth=1):
        ...
        a0 = to_categorical(0,2)
        a1 = to_categorical(1,2)
        #a2 = to_categorical(2,2)
        #a3 = to_categorical(3,2)
        ...
        l_max = [max0, max1, max2, max3]
        l_amax = [arg_max0, arg_max1, arg_max2, arg_max3]
        #print(l_amax)
        if(action != l_amax[np.argmax(l_max)]):
            self.difference += 1
        '''
        l_max = [max0, max1]
        l_amax = [arg_max0, arg_max1]
        # print(l_amax)
        if (action != l_amax[np.argmax(l_max)]):
            print("DIVERSO!!!")
    ...
    def take_step(self, mode='train'):
        ...
        enc_s1 = self.encoder(torch.from_numpy(np.asarray(s_1)).type(torch.FloatTensor), 0, 50)
